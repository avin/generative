...
vec3 distortFunct(vec3 transformed) {
  float radiusVariation = -noise(transformed * radiusNoiseFrequency + iTime + 100.) * radiusVariationAmplitude;

  return normalize(transformed) * (radiusVariation + radius);
}

vec3 orthogonal2(vec3 v) { return cross(v, vec3(0.0, 0.0, 1.0)); }
...
vec3 distortNormal(vec3 position, vec3 distortedPosition, vec3 normal) {
  // vec3 tangent1 = normalize(cross(normal, vec3(1.0, 0.0, 0.0)) * cross(normal, vec3(0.0, 1.0, 0.0)));

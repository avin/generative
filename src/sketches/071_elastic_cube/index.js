...
const sketch = async ({ canvas, width, height }) => {
  ...
  const updatePositions = (time, mesh, pl) => {
    ...
    for (let i = 0; i < positions.length / 3; i += 1) {
      ...
      if (pm % 2 !== 0) {
        const rpf = Math.max(Math.min(pf + (z - 0.5) * dF, 1), 0);
        const q = Quaternion.RotationAxis(new Vector3(0, 0, 1), (Math.cos(rpf * Math.PI) * Math.PI) / 2);
        vec.rotateByQuaternionAroundPointToRef(q, new Vector3(0, 0, z), vec);
      } else {
        const rpf = Math.max(Math.min(pf + (y - 0.5) * dF, 1), 0);
        const q = Quaternion.RotationAxis(new Vector3(0, 1, 0), (Math.cos(rpf * Math.PI) * Math.PI) / 2);
        vec.rotateByQuaternionAroundPointToRef(q, new Vector3(0, y, 0), vec);
      }

      pl.chromaticAberration.aberrationAmount = Math.sin(((t / 2) % 1) * Math.PI * 2) * 80;
